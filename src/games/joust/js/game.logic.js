...
const logicOfGame = {
    ...
    getFitnessFunctionNofGames() {
        return 100;
    },
    getBoardAsNeuralNetworkInput(state) {
        let inputBoard = [...Array(boardHeight * boardWidth)].fill(0);
        inputBoard[state.player1[1] * boardWidth + state.player1[0]] = 1;
        inputBoard[state.player2[1] * boardWidth + state.player2[0]] = 2;
        for (let [x, y] of state.removed) {
            inputBoard[y * boardWidth + x] = -1;
        }
        return inputBoard;
    },
};
